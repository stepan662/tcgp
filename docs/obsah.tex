...
Vidíme, že přidávání a odebírání závorek nám může připomínat zásobník.
A takto podobně opravdu zásobníkový automat funguje.\\
...
Ukažme si, jak může vypadat zpracování řetězce \texttt{'(())'} bezkontextovou
gramatikou $G$ (z kapitoly \ref{subsec:contextFreeGrammars}) pomocí LL syntaktické analýzy.
Vlevo je znázorněný zásobník (s vrcholem)
a šipky nám indikují, v jaké části řetězce se právě nacházíme.
...
Při zpracování postupujeme podle daného algoritmu LL syntaktické analýzy, nejdříve jej ukážeme na
tomto příkladu. Před zahájením algoritmu vložíme na zásobník symbol '\$',
který označuje konec souboru a počáteční symbol 'S'. Nyní ukažme pár prvních kroků (číslování odpovídá obrázku):

\begin{enumerate}
  \item Na zásobníku je neterminál 'S' - na základě vstupu '(' použijeme pravidlo 1 a nahradíme S za pravou stranu pravidla
  \item Na zásobníku je terminál '(' a shoduje se se vstupním symbolem - můžeme jej odstranit
  \item Na zásobníku je neterminál 'E' a na vstupu opět '(', použijeme pravidlo 2
  \item Na zásobníku je terminál '(' a shoduje se se vstupem - odstraníme
  \item Na zásobníku je neterminál 'E' a na vstupu ')' - použijeme pravidlo 3, což nám odstraní E ze zásobníku
  \item ...
\end{enumerate}

Které pravidlo použít rozhodujeme v tomto případě intuitivně, v případě implementace
však potřebujeme jasnou odpověď. K tomu slouží tzv. LL tabulka, která nám na základě
...
\begin{algorithm}[H]
  ...
  \Begin{
    ...
    \Repeat{žádná z množin Empty nezměněna} {
      \If{$A \rightarrow X_1 X_2 ... X_n \in P$ a $Empty(X_i) = {\varepsilon}$
              pro všechna $i = 1, ... ,n$}{
        $Empty(A)$ := $\{\varepsilon\}$\;
      }
    }
...
\noindent
Množina $Predict(A \rightarrow x)$ říká, které terminální mohou být
...
\begin{defn}
  ...
  \noindent
  Jinak pokud $Empty(x) = \emptyset$ potom:\\
    \indent
