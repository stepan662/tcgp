...
\term{syntaktické analyzátory} nebo \term{parsery}.\\
...
znázornit jako strom, jenž poté může být základem \term{sémantické analýzy},
tedy jakéhosi \uv{pochopení} přijímaného řetězce.
Teorie formálních jazyků je základním kamenem při návrhu překladačů
jazyků programovacích. Ty většinou spadají do \term{bezkontextových jazyků},
...
Hlavní částí práce je kapitola \ref{chap:TreeLangs}, kde je uvedena metoda kontroly
...
jako ten původní. Takové automaty se označují jako \term{ekvivalentní}.
...
\begin{algorithm}[H]
  ...
  \KwIn{Konečný automat $M = (Q, \Sigma, R, s, F)$ a $E \subseteq Q$}
...
\begin{algorithm}[H]
  ...
  \KwIn{Konečný automat $I = (Q_I, \Sigma_I, R_I, s_I, F_I)$ a $E \subseteq Q$}
...
otevírací závorku, přešli jsme do stavu \uv{o jedna více závorek}, v případě uzavírací závorky
...
Poté je zde ještě otázka postupu při samotném zpracování řetězce.
...
Hlavní součástí je \term{LL tabulka}, která nám na základě
...
\begin{itemize}
  ...
  z LL tabulky pravidlo 1 a nahradíme \symb{S} za pravou stranu pravidla
...
že porovnáme prioritu operátoru v \term{reduce} pravidle se vstupním symbolem
...
gramatiku z příkladu \ref{exmp:WW} a řetězec \symb{01\#11}:
...
\begin{exmp}
  ...
  \term{reduce}, tedy že \symb{S} má větší prioritu než \symb{a}. Při dalším konfliktu nastane
  tato situace:
  ...
  my ovšem potřebujeme, aby se provedla operace \term{shift}. Pomocí priorit lze
...
Musíme rozlišovat situace, kdy víme jistě,
      ...
      \ForAll{znak $c \in a$}{
        \ForAll{stav $q \in$ stavy}{
          \uIf{přechod $qc \rightarrow q_{new} \in R$}{
            V množině $Q$ nahraď $q$ za $q_{new}$\;
          }
          \Else{
            Odeber $q$ z $Q$\;
          }
...
\begin{itemize}
  ...
  \item Pokud jde o \term{reduce-reduce} konflikt, vyzkoušíme všechny možnosti a
...
Tato práce se zaměřila na vývoj syntaktického analyzátoru, který zpracovává
...
V sekci \ref{sec:LLTreeLevelCheck} jsme upravili LL syntaktickou
...
Jelikož však LR syntaktická analýza sestavuje derivační strom zdola a
...
některých konfliktů v LR tabulce, a to jak \term{shift-reduce}, tak i \term{reduce-reduce}.
...
Pomocí metod uvedených v této části lze zpracovávat i jazyky s nedeterministickou
řízenou gramatikou, kde není možné konflikty řešit pomocí priority operátorů.
Jako příklad takové gramatiky je uveden jazyk $a^{2^n}$, jenž jsme tímto přístupem
schopni zpracovat.\\

Implementační část vychází z teoretických poznatků a představuje syntaktický analyzátor
umožňující zpracovávání velké části \term{jazyků řízených stromy}.
...
Výslednou sílu lze ohraničit takto: je vyšší než u \term{bezkontextových deterministických gramatik}
a nižší oproti \term{neomezeným gramatikám}.
...
Pro zlepšení uživatelské přívětivosti by se také mohlo provést odstranění
...
Výsledná aplikace slouží hlavně pro demonstrační účely, ale použitá
řešení by bylo možné využít např. pro specifické konstrukce programovacího jazyka,
které nomohou být vyjádřeny pomocí bezkontextové gramatiky.
